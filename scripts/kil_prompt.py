...
def build_kil_prompt(context: Dict[str, Any]) -> str:
    ...
    return (
        ...
        "固定ルール（厳守）:\n"
        "- PowerShell前提では `&&` / `||` / bash依存構文を提案しない。\n"
        "- 機密情報(APIキー・トークン・パスワード)を生テキストで提案・再構築しない。\n"
        "- `AGENT_BRAIN` へ反映するなら、変更意図・制約・未解決点を再利用可能な形で簡潔に残す。\n"
        "- 解析対象がCI/Hook設定/認証・インフラ設定の場合は、運用リスクを高めに評価する。\n\n"
        "出力ルール:\n"
