...
description: kintoneスペースのスレッドコメントを検索・削除するスキル。「この投稿を消して」「スレッドのコメントを削除」「誤投稿を取り消し」などの依頼に使用。まずサンドボックス（space=81/thread=131）でドライラン/削除手順を確認してから本番へ。
---
...
- サンドボックスで動作確認してから本番へ（本番削除は `--allow-production` が必要）。
- 1件だけ確実に消す場合は `--comment-id` を推奨（`--match-text` は複数件にヒットし得る）。

## クイックスタート

```powershell
# 1) サンドボックスで候補確認（削除しない）
