...
- 公式ヘルプ（`https://ichiba.faq.rakuten.net/detail/000006734`）の要点（差分）:
  - 発行対象: 「お支払い確定」か、`kobo`/楽天ブックス（ダウンロード）等の非出荷注文なら「注文処理済み」
  - 発行方法: 購入履歴詳細の「領収書・請求書」から再発行可能（回数制限なし）
  - 決済種別別の発行物:
    - カード/銀行振込/前払(コンビニ・ATM)/後払い/Apple Pay/PayPal: インボイス対応領収書
    - 代引き/請求書払い/ショッピングクレジット/リース等: インボイス対応請求書
  - 不可条件例: ふるさと納税は購入履歴から発行不可
  - 決済手段は「除外判定」ではなく「帳票種別（領収書/請求書）」として記録し、再取得判断に利用する。
...
- 補足:
  - Amazon公式ヘルプページは環境上 Web 経由の自動取得で `503` を返すため、文言確認は手動実行時に併走させる（`nodeId=201894740` 系）
- 実装反映方針:
...
- P1: 個別明細の取りこぼし、`no_receipt_payment_method` の増加（当日中の手動補完）
...
## 8. 今回の公式照合結果（追加）
- 追記: 2026-02-15 の最終確認
- 楽天公式: 「領収書・請求書について」は、決済方法別に領収書か請求書かを明確化。
  - 代引きや請求書払いなどは請求書発行が基本（経費起票側の分類ルールを運用で明記）
...
- 定例再確認（週次）手順を固定:
  - 注文履歴 → 注文の詳細 → 領収書/購入明細 の遷移が正しく動作するか
  - 代金引換のみで no_receipt 判定が成立するか
  - 明細書/適格請求書 と 印刷可能な注文概要（legacy/new format）の文言・URL差分
- 変更検知時の扱い:
