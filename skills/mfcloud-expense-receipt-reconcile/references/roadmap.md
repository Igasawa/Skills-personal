...
- ダウンロード実行中の進捗ログ更新（Amazon/楽天）
- 取得不能注文が「理由付き」で記録されること（黙って欠損しない）
- 楽天ブックス等の領収書非対応ケースを `no_receipt` として処理できること

Phase 1.5（MF下書き作成）で残っていること:
- Step0（preflight + 連携更新）後に `outgo_input` へ遷移し、対象月の連携明細が表示されること（開始条件の現物確認）
...
## 実装優先順位
1. ダウンロード進捗ログ更新（Amazon/楽天）を実装し、長時間実行でも状態が追えるようにする
2. 取得不能注文を理由付きで記録し、欠損ゼロの監査可能性を上げる（Amazon/楽天）
3. 楽天の領収書非対応ケースを `no_receipt` として取り扱えるようにする
4. 「保存して印刷準備」体験で誤表示（成功/失敗）が出ないことを保証する（運用安定性）
5. Phase 1（Amazon/楽天）のMVP受け入れテスト整備（チェックリストの自動化/現物確認手順の固定）
6. Phase 1.5（MF下書き作成）: Step0の開始条件（`outgo_input` 表示）を確実に満たす
7. Phase 1.5（MF下書き作成）: 領収書添付の安定化と、失敗時の行単位ログ追跡（`reports/audit_log.jsonl` + `_runs/*.log`）
8. Step 4.5（手動取り込み）: `manual/orders.jsonl` 反映とStep5突合までの導線を受け入れ条件として固定
9. Phase 2の共通アダプタ仕様策定
10. Aqua Voice -> ChatGPT -> Claude -> Replit の順で実装
